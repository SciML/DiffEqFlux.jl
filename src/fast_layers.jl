...
function applychain(fs::Tuple, x, p)
    applychain(Base.tail(fs), first(fs)(x, p[1:paramlength(first(fs))]),
...
struct FastDense{F, F2, C} <: FastLayer
    ...
    function FastDense(in::Integer, out::Integer, σ = identity;
        ...
        _σ = NNlib.fast_act(σ)
        new{typeof(_σ), typeof(initial_params), typeof(cache)}(out, in, _σ, initial_params,
                                                               cache, bias, numcols)
    end
end

# To work with scalars(x::Number)
function (f::FastDense)(x::Number, p)
    ((f.bias == true) ?
...
function (f::FastDense)(x::AbstractVector, p)
    ((f.bias == true) ?
     (f.σ.(reshape(p[1:(f.out * f.in)], f.out, f.in) * x .+ p[(f.out * f.in + 1):end])) :
     (f.σ.(reshape(p[1:(f.out * f.in)], f.out, f.in) * x)))
end
...
function (f::FastDense)(x::AbstractMatrix, p)
    ((f.bias == true) ?
     (f.σ.(reshape(p[1:(f.out * f.in)], f.out, f.in) * x .+ p[(f.out * f.in + 1):end])) :
     (f.σ.(reshape(p[1:(f.out * f.in)], f.out, f.in) * x)))
end
...
ZygoteRules.@adjoint function (f::FastDense)(x::AbstractMatrix, p)
    if typeof(f.cache) <: Nothing
        ...
        @view(f.cache.y[:, 1:f.cache.cols[1]]) .= f.σ.(@view(f.cache.r[:,
                                                                       1:f.cache.cols[1]]))
    end
    function FastDense_adjoint(ȳ)
        if typeof(f.cache) <: Nothing
            ...
            if typeof(f.σ) <: typeof(NNlib.tanh_fast)
                @view(f.cache.zbar[:, 1:f.cache.cols[1]]) .= ȳ .* (1 .-
                                                              @view(f.cache.y[:,
                                                                              1:f.cache.cols[1]]) .^
                                                              2)
            elseif typeof(f.σ) <: typeof(identity)
            ...
            mul!(@view(f.cache.xbar[:, 1:f.cache.cols[1]]), f.cache.W',
                 @view(f.cache.zbar[:, 1:f.cache.cols[1]]))
            f.cache.pbar .= if f.bias == true
                vec(vcat(vec(f.cache.Wbar),
                         sum(@view(f.cache.zbar[:, 1:f.cache.cols[1]]), dims = 2)))# bbar = zbar
            else
                vec(f.cache.Wbar)
            end
...
struct StaticDense{out, in, bias, F, F2} <: FastLayer
    ...
    function StaticDense(in::Integer, out::Integer, σ = identity;
                         bias::Bool = true, initW = Flux.glorot_uniform,
                         initb = Flux.zeros32)
        ...
        temp = ((bias == true) ? vcat(vec(initW(out, in)), initb(out)) :
...
function Cassette.overdub(ctx::DiffEqSensitivity.HasBranchingCtx,
                          f::StaticDense{out, in, bias}, x, p) where {out, in, bias}
