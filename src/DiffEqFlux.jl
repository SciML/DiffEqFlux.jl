...
ZygoteRules.@adjoint function ZygoteRules.literal_getproperty(d::ForwardDiff.Dual{T},
                                                              ::Val{:partials}) where {T}
...
ZygoteRules.@adjoint function ZygoteRules.literal_getproperty(A::Tridiagonal, ::Val{:dl})
    A.dl, y -> Tridiagonal(dl, zeros(length(d)), zeros(length(du)))
end
ZygoteRules.@adjoint function ZygoteRules.literal_getproperty(A::Tridiagonal, ::Val{:d})
    A.d, y -> Tridiagonal(zeros(length(dl)), d, zeros(length(du)))
end
ZygoteRules.@adjoint function ZygoteRules.literal_getproperty(A::Tridiagonal, ::Val{:du})
    A.dl, y -> Tridiagonal(zeros(length(dl)), zeros(length(d), du))
end
ZygoteRules.@adjoint function Tridiagonal(dl, d, du)
    Tridiagonal(dl, d, du),
    p̄ -> (diag(p̄[2:end, 1:(end - 1)]), diag(p̄), diag(p̄[1:(end - 1), 2:end]))
end
...
function Flux.device(::FastLayer)
    @warn "device(f::FastLayer) is a no-op: to move FastChain computations to a GPU, apply gpu(x) to the weight vector"
end
function Flux.gpu(::FastLayer)
    @warn "device(f::FastLayer) is a no-op: to move FastChain computations to a GPU, apply gpu(x) to the weight vector"
end
function Flux.cpu(::FastLayer)
    @warn "device(f::FastLayer) is a no-op: to move FastChain computations to a CPU, apply cpu(x) to the weight vector"
end

export DeterministicCNF, FFJORD, NeuralODE, NeuralDSDE, NeuralSDE, NeuralCDDE, NeuralDAE,
