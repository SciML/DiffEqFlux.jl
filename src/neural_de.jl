...
struct NeuralODE{M, P, RE, T, A, K} <: NeuralDELayer
    ...
    function NeuralODE(model, tspan, args...; p = nothing, kwargs...)
        ...
        new{typeof(model), typeof(p), typeof(re),
            typeof(tspan), typeof(args), typeof(kwargs)}(model, p, re, tspan, args, kwargs)
    end

    function NeuralODE(model::FastChain, tspan, args...; p = initial_params(model),
                       kwargs...)
        re = nothing
        new{typeof(model), typeof(p), typeof(re),
            typeof(tspan), typeof(args), typeof(kwargs)}(model, p, re, tspan, args, kwargs)
    ...
    function NeuralODE(model::Lux.AbstractExplicitLayer, tspan, args...; p = nothing,
                       kwargs...)
        re = nothing
        new{typeof(model), typeof(p), typeof(re),
            typeof(tspan), typeof(args), typeof(kwargs)}(model, p, re, tspan, args, kwargs)
    ...
    function NeuralDSDE(model1, model2, tspan, args...; p = nothing, kwargs...)
        ...
        new{typeof(model1), typeof(p), typeof(re1), typeof(model2), typeof(re2),
            typeof(tspan), typeof(args), typeof(kwargs)}(p,
                                                         length(p1), model1, re1, model2,
                                                         re2, tspan, args, kwargs)
    end
...
struct NeuralSDE{P, M, RE, M2, RE2, T, A, K} <: NeuralDELayer
    ...
    function NeuralSDE(model1, model2, tspan, nbrown, args...; p = nothing, kwargs...)
        ...
        new{typeof(p), typeof(model1), typeof(re1), typeof(model2), typeof(re2),
            typeof(tspan), typeof(args), typeof(kwargs)}(p, length(p1), model1, re1, model2,
                                                         re2, tspan, nbrown, args, kwargs)
    end
    ...
    function NeuralSDE(model1::Lux.AbstractExplicitLayer, model2::Lux.AbstractExplicitLayer,
                       tspan, nbrown, args...;
                       p1 = nothing, p = nothing, kwargs...)
        re1 = nothing
        re2 = nothing
        new{typeof(p), typeof(model1), typeof(re1), typeof(model2), typeof(re2),
            typeof(tspan), typeof(args), typeof(kwargs)}(p, Int(1), model1, re1, model2,
                                                         re2, tspan, nbrown, args, kwargs)
    end
end
...
function (n::NeuralSDE)(x, p = n.p)
    ...
    ff = SDEFunction{false}(dudt_, g, tgrad = basic_tgrad)
    prob = SDEProblem{false}(ff, g, x, n.tspan, p,
                             noise_rate_prototype = zeros(Float32, length(x), n.nbrown))
    solve(prob, n.args...; sensealg = TrackerAdjoint(), n.kwargs...)
end
...
function (n::NeuralSDE{P, M})(x, p, st) where {P, M <: Lux.AbstractExplicitLayer}
    ...
    prob = SDEProblem{false}(ff, g, x, n.tspan, p,
                             noise_rate_prototype = zeros(Float32, length(x), n.nbrown))
    solve(prob, n.args...; sensealg = InterpolatingAdjoint(), n.kwargs...),
    (state1 = st1, state2 = st2)
end
    ...
    function NeuralCDDE(model, tspan, hist, lags, args...; p = nothing, kwargs...)
        ...
        new{typeof(p), typeof(model), typeof(re), typeof(hist), typeof(lags),
            typeof(tspan), typeof(args), typeof(kwargs)}(p, model,
                                                         re, hist, lags, tspan, args,
                                                         kwargs)
    end

    function NeuralCDDE(model::FastChain, tspan, hist, lags, args...;
...
struct NeuralDAE{P, M, M2, D, RE, T, DV, A, K} <: NeuralDELayer
    ...
    function NeuralDAE(model, constraints_model, tspan, du0 = nothing, args...; p = nothing,
        ...
        new{typeof(p), typeof(model), typeof(constraints_model),
            ...
            typeof(differential_vars), typeof(args), typeof(kwargs)}(model,
                                                                     constraints_model, p,
                                                                     du0, re, tspan,
                                                                     differential_vars,
                                                                     args, kwargs)
