...
"""
struct DeterministicCNF{M, P, RE, D, T, A, K} <:
       CNFLayer where {M, P <: AbstractVector{<:AbstractFloat}, RE <: Function,
                       D <: Distribution, T, A, K}
    model::M
    p::P
    re::RE
    basedist::D
...
function DeterministicCNF(model, tspan, args...;
                          p::P = nothing, basedist::D = nothing,
                          kwargs...) where {
                                            P <:
                                            Union{AbstractVector{<:AbstractFloat}, Nothing},
                                            RE <: Function,
...
@deprecate DeterministicCNF(model, p, re, basedist, tspan, args, kwargs) FFJORD(model, p,
                                                                                re,
                                                                                basedist,
                                                                                tspan,
                                                                                monte_carlo = false,
...
"""
struct FFJORD{M, P, ST, RE, D, T, A, K} <:
       CNFLayer where {M, P <: AbstractVector{<:AbstractFloat}, ST,
                       RE <: Union{Function, Lux.AbstractExplicitLayer}, D <: Distribution,
                       T, A, K}
    model::M
    p::P
    st::ST
    ...
    function FFJORD(model::Lux.Chain, tspan, args...; p = nothing, st = nothing,
        ...
        new{typeof(model), typeof(p), typeof(st), typeof(re),
            typeof(basedist), typeof(tspan), typeof(args), typeof(kwargs)}(model, p, st, re,
                                                                           basedist, tspan,
                                                                           args, kwargs)
    end

    function FFJORD(model, tspan, args...; p = nothing, st = nothing, basedist = nothing,
                    kwargs...)
        #
        _p, re = Flux.destructure(model)
        if isnothing(p)
