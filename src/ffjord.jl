...
"""
struct DeterministicCNF{M,P,RE,D,T,A,K} <: CNFLayer where {
    M,
...
function DeterministicCNF(
    ...
    if isnothing(basedist)
        ...
        basedist =
            MvNormal(zeros(type_input, size_input), Diagonal(ones(type_input, size_input)))
    end
    DeterministicCNF(model, p, re, basedist, tspan, args, kwargs)
end

@deprecate DeterministicCNF(model, p, re, basedist, tspan, args, kwargs) FFJORD(
    model,
    p,
    re,
    basedist,
...
"""
struct FFJORD{M,P,ST,RE,D,T,A,K} <: CNFLayer where {
    M,
    P<:AbstractVector{<:AbstractFloat},
    ST,
    RE<:Union{Function,Lux.AbstractExplicitLayer},
    D<:Distribution,
    T,
    A,
    K,
...
function ffjord(u, p, t, re, e, st; regularize = false, monte_carlo = true)
    m = re(p)
    if regularize
        z = u[1:end-3, :]
        if monte_carlo
...
function ffjord(u, p, t, re::Lux.Chain, e, st; regularize = false, monte_carlo = true)
    if regularize
        z = u[1:end-3, :]
        if monte_carlo
...
FFJORDDistribution(model; regularize = false, monte_carlo = true) =
...
Distributions._logpdf(d::FFJORDDistribution, x::AbstractArray) =
    forward_ffjord(d.model, x; d.regularize, d.monte_carlo)[1]
Distributions._rand!(rng::AbstractRNG, d::FFJORDDistribution, x::AbstractVector{<:Real}) =
    (x[:] = backward_ffjord(d.model, size(x, 2); d.regularize, d.monte_carlo, rng))
Distributions._rand!(rng::AbstractRNG, d::FFJORDDistribution, A::DenseMatrix{<:Real}) =
