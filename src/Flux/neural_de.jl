...
function neural_ode_rd(model,x,tspan,
                       args...;kwargs...)
  dudt_(u,p,t) = model(u)
  _x = x isa TrackedArray ? x : Tracker.param(x)
  prob = ODEProblem{false}(dudt_,_x,tspan)
  solve(prob, args...; kwargs...) |> Array
end
...
function neural_dmsde(model,x,mp,tspan,
  ...
  dudt_(u,p,t) = model(u)
  g(u,p,t) = mp.*u
  prob = SDEProblem{false}(dudt_,g,param(x),tspan,nothing)
