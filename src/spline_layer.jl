...
"""
struct SplineLayer{T <: Tuple{Real, Real}, R <: Real, S1 <: AbstractVector, S2 <: UnionAll
                   } <: AbstractSplineLayer
    time_span::T
    time_step::R
    saved_points::S1
    ...
    function SplineLayer(time_span, time_step, spline_basis, saved_points = nothing)
        ...
        new{typeof(time_span), typeof(time_step), typeof(saved_points), typeof(spline_basis)
            }(time_span, time_step, saved_points, spline_basis)
    end
end
