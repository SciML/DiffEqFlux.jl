...
@testset "Solver: $(nameof(typeof(solver)))" for solver in (KenCarp4(),
    Rodas5(), RadauIIA5())
    neuralde = NeuralODE(model, tspan, solver; saveat = t, reltol = 1e-7, abstol = 1e-9)
    ps, st = Lux.setup(Xoshiro(0), neuralde)
    ps = ComponentArray(ps)
    lux_model = Lux.Experimental.StatefulLuxLayer(neuralde, ps, st)
    loss1 = loss_n_ode(lux_model, ps)
    optfunc = Optimization.OptimizationFunction((x, p) -> loss_n_ode(lux_model, x),
        Optimization.AutoZygote())
    optprob = Optimization.OptimizationProblem(optfunc, ps)
    res = Optimization.solve(optprob, Adam(0.1); callback = callback(solver),
        maxiters = 100)
