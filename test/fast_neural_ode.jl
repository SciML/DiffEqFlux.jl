...
fastdudt2 = FastChain((x, p) -> x .^ 3, FastDense(2, 50, tanh), FastDense(50, 2))
...
dudt2 = Flux.Chain((x) -> x .^ 3, Flux.Dense(2, 50, tanh), Flux.Dense(50, 2))
...
@test Zygote.gradient((p) -> fast_loss_n_ode(p)[1], p)[1] ≈
      Zygote.gradient((p) -> loss_n_ode(p)[1], p)[1] rtol = 4e-3
