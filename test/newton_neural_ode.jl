...
res = DiffEqFlux.sciml_train(loss_function, nODE.p, NewtonTrustRegion(),
                             Optimization.AutoZygote(), maxiters = 100, cb = cb) #ensure backwards compatibility of `cb`
@test loss_function(res.minimizer) < l1
res = DiffEqFlux.sciml_train(loss_function, nODE.p, Optim.KrylovTrustRegion(),
                             Optimization.AutoZygote(), maxiters = 100, callback = cb)
@test loss_function(res.minimizer) < l1
