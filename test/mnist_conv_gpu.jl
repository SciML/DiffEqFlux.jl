...
function loadmnist(batchsize = bs, train_split = 0.9)
    ...
    return (
        ...
        DataLoader(
            gpu.(collect.((x_train, y_train)));
            batchsize = batchsize,
            shuffle = true,
        ),
        # Don't shuffle the test data
        DataLoader(
            gpu.(collect.((x_test, y_test)));
            batchsize = batchsize,
            shuffle = false,
        ),
    )
...
cb() = begin
    ...
    if iter % 10 == 1
        ...
        test_accuracy =
            accuracy(model, test_dataloader; n_batches = length(test_dataloader)) * 100
        @printf(
            "Iter: %3d || Train Accuracy: %2.3f || Test Accuracy: %2.3f\n",
            iter,
            train_accuracy,
            test_accuracy
        )
