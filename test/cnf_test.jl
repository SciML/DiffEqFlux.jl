...
@testset "Smoke test for FFJORD" begin
    ...
    @testset "AutoForwardDiff as adtype" begin
        ...
        @testset "regularize=false & monte_carlo=false" begin
            ...
            @test !isnothing(DiffEqFlux.sciml_train(θ -> loss(θ; regularize, monte_carlo),
                                                    ffjord_mdl.p, ADAM(1.0f-1), adtype;
        ...
        @testset "regularize=true & monte_carlo=false" begin
            ...
            @test_broken !isnothing(DiffEqFlux.sciml_train(θ -> loss(θ; regularize,
                                                                     monte_carlo),
                                                           ffjord_mdl.p, ADAM(1.0f-1),
                                                           adtype; callback = callback,
                                                           maxiters = 10))
        end
...
@testset "Smoke test for FFJORDDistribution (sampling & pdf)" begin
    ...
    res = DiffEqFlux.sciml_train(θ -> loss(θ; regularize, monte_carlo), ffjord_mdl.p,
                                 ADAM(1.0f-1), adtype; callback = callback, maxiters = 10)
    ffjord_d = FFJORDDistribution(FFJORD(nn, tspan, Tsit5(); p = res.u); regularize,
                                  monte_carlo)
...
@testset "Test for default base distribution and deterministic trace FFJORD" begin
    ...
    adtype = Optimization.AutoZygote()
    res = DiffEqFlux.sciml_train(loss, ffjord_mdl.p, ADAM(1.0f-1), adtype;
...
@testset "Test for alternative base distribution and deterministic trace FFJORD" begin
    ...
    adtype = Optimization.AutoZygote()
    res = DiffEqFlux.sciml_train(loss, 0.01f0 * ffjord_mdl.p, ADAM(1.0f-1), adtype;
                                 callback = callback, maxiters = 100)
