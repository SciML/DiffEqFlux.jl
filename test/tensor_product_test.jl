...
function run_test(f, layer, atol)
    ...
    res = DiffEqFlux.sciml_train(loss_function, layer.p, ADAM(0.1), GalacticOptim.AutoZygote(), cb=cb, maxiters = 100)
    res = DiffEqFlux.sciml_train(loss_function, res.minimizer, ADAM(0.01), GalacticOptim.AutoZygote(), cb=cb, maxiters = 100)
    res = DiffEqFlux.sciml_train(loss_function, res.minimizer, BFGS(), GalacticOptim.AutoZygote(), cb=cb, maxiters = 200)
    opt = res.minimizer
