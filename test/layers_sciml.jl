...
pmin = DiffEqFlux.sciml_train(loss_rd, p, BFGS(initial_stepnorm = 0.01), cb = cb, diffmode=DiffEqFlux.ForwardDiffMode())
loss2 = loss_rd(pmin.minimizer)
@test 10loss2 < loss1

pmin = DiffEqFlux.sciml_train(loss_rd, p, BFGS(initial_stepnorm = 0.01), cb = cb, diffmode=DiffEqFlux.TrackerDiffMode())
loss2 = loss_rd(pmin.minimizer)
@test 10loss2 < loss1

pmin = DiffEqFlux.sciml_train(loss_rd, p, BFGS(initial_stepnorm = 0.01), cb = cb, diffmode=DiffEqFlux.ReverseDiffMode())
loss2 = loss_rd(pmin.minimizer)
@test 10loss2 < loss1
