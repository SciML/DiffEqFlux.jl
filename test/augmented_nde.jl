...
fastdudt22 = FastChain(FastDense(4, 50, tanh), FastDense(50, 16),
                       (x, p) -> reshape(x, 4, 4))
fastddudt = FastChain(FastDense(12, 50, tanh), FastDense(50, 4))

# Augmented Neural ODE
anode = AugmentedNDELayer(NeuralODE(fastdudt, tspan, Tsit5(), save_everystep = false,
                                    save_start = false), 2)
anode(x)
...
andsde = AugmentedNDELayer(NeuralDSDE(fastdudt, fastdudt2, (0.0f0, 0.1f0), SOSRI(),
...
asode = AugmentedNDELayer(NeuralSDE(fastdudt, fastdudt22, (0.0f0, 0.1f0), 4, LambaEM(),
                                    saveat = 0.0:0.01:0.1), 2)
asode(x)
...
adode = AugmentedNDELayer(NeuralCDDE(fastddudt, (0.0f0, 2.0f0), (p, t) -> zeros(Float32, 4),
                                     (1.0f-1, 2.0f-1),
