...
nn = FastChain((x, p) -> x .^ 3, FastDense(2, 16, tanh), FastDense(16, 2))
...
res_single_shooting =
    DiffEqFlux.sciml_train(loss_single_shooting, neuralode.p, ADAM(0.05), maxiters = 300)
...
res_ms =
    DiffEqFlux.sciml_train(loss_multiple_shooting, neuralode.p, ADAM(0.05), maxiters = 300)
...
res_ms_abs2 = DiffEqFlux.sciml_train(
    loss_multiple_shooting_abs2,
    neuralode.p,
    ADAM(0.05),
    maxiters = 300,
)
...
res_ms_fd = DiffEqFlux.sciml_train(
    loss_multiple_shooting_fd,
    neuralode.p,
    ADAM(0.05),
    maxiters = 300,
)
...
res_ms_ensembles = DiffEqFlux.sciml_train(
    loss_multiple_shooting_ens,
    neuralode.p,
    ADAM(0.05),
    maxiters = 300,
)
