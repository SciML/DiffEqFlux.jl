...
@testset "adjoint mode" begin
    ...
    node = NeuralODE(fastdudt,tspan,Tsit5(),saveat=0.0:0.1:10.0)
    grads = Zygote.gradient(()->sum(node(x)),Flux.params(x,node))
    @test ! iszero(grads[x])
    @test ! iszero(grads[node.p])
...
@testset "Tracker mode" begin
    ...
    NeuralODE(fastdudt,tspan,Tsit5(),saveat=0.0:0.1:10.0,sensealg=TrackerAdjoint())
    grads = Zygote.gradient(()->sum(node(x)),Flux.params(x,node))
    @test ! iszero(grads[x])
    @test ! iszero(grads[node.p])
...
fastdudt22 = FastChain(FastDense(2,50,tanh),FastDense(50,4),x->reshape(x,2,2))
